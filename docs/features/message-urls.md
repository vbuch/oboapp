...
The document ID **is** the slug — there is no separate `slug` field.
...
The system checks for duplicates before storing a new message:

1. Generate random 8-character ID
2. Check Firestore for existing document with that ID
...
**GET** `/api/messages/by-slug?slug={id}`

Fetches a message by its document ID (which is the slug).
