...
/**
 * Returns true if any coordinate in a GeoJSON feature collection falls within
 * any of the supplied interest zones.
 */
export const isMessageInAnyInterestZone = (
  geoJson: GeoJSONFeatureCollection | undefined | null,
  zones: readonly { coordinates: { lat: number; lng: number }; radius: number }[],
): boolean => {
  if (!geoJson?.features || zones.length === 0) return false;
  ...
  for (const feature of geoJson.features) {
    ...
    if (geometry.type === "Point") {
      ...
      for (const c of geometry.coordinates) coords.push(c as [number, number]);
  ...
  return coords.some(([lng, lat]) =>
    zones.some(
      (zone) =>
        distanceMetres(lat, lng, zone.coordinates.lat, zone.coordinates.lng) <=
        zone.radius,
    ),
  );
};
