...
export default function InterestContextMenu({
  ...
  return (
    <>
      ...
      <button
        type="button"
        className={`fixed inset-0 ${zIndex.overlayContent} bg-black/20 backdrop-blur-sm pointer-events-auto cursor-default`}
        onClick={onClose}
        aria-label="Затвори менюто"
      />
      {/* Menu */}
      <div
        className={`fixed ${zIndex.modalBackdrop} bg-white rounded-lg shadow-xl border border-neutral-border py-1 min-w-[150px]`}
        style={{
          left: `${position.x}px`,
          top: `${position.y}px`,
        }}
