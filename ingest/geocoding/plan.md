...
- [x] Step 5: Create `ingest/geocoding/overpass/geocoder.ts` implementing `StreetGeocoder`
...
- [x] Step 8a: `ingest/geocoding/google/geocoder.ts` implementing `PinGeocoder`
  - [x] Wraps `google/service.ts` `geocodeAddresses()`; returns `PinResult | null`
  - [x] No cache lookup (removed)

- [x] Step 8b: `ingest/geocoding/overpass/geocoder.ts` implementing `PinGeocoder`
  - [x] Wraps `overpass/service.ts` `overpassGeocodeAddresses()`; returns `PinResult | null`
  - [x] Also implements `StreetGeocoder` (single provider class for compatible interfaces)

- [x] Step 8c: `ingest/geocoding/cadastre/geocoder.ts` implementing `CadastralGeocoder`
  - [x] Wraps `cadastre/service.ts` `geocodeCadastralProperties()`

- [x] Step 8d: `ingest/geocoding/gtfs/geocoder.ts` implementing `BusStopGeocoder`
  - [x] Implementation is GTFS-standard (uses `stop_code`, `lat`/`lng` from `stops.txt`). The only Sofia-specific artifact is the `"Спирка ${stopCode}"` label which is a minor localization string, not the protocol. The GTFS URL is configuration (in `geocoding-sources.ts`), not baked in.
  - [x] Wraps `gtfs/geocoding-service.ts`; Google fallback as second entry in the `busStop` provider array

- [x] Step 8e: `ingest/geocoding/educational-facilities/geocoder.ts` implementing `EducationalFacilityGeocoder`
  - [x] Wraps `educational-facilities/geocoding-service.ts`; Google fallback as second entry

- [x] Step 9: Register all in `providers.ts`

```
pin: [new GoogleGeocoder(), new OverpassGeocoder()]
street: [new OverpassGeocoder()]
cadastral: [new CadastreGeocoder()]
busStop: [new GtfsGeocoder(), new GoogleGeocoder()]
educationalFacility: [new EducationalFacilitiesGeocoder(), new GoogleGeocoder()]
```
