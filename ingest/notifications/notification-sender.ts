...
export function buildNotificationPayload(
  ...
  // Use plainText if available (no markdown), otherwise strip markdown from text
  const textForNotification = message.plainText
    ? message.plainText
    : stripMarkdown(message.text);

  const messagePreview =
    textForNotification.length > 100
      ? textForNotification.substring(0, 100) + "..."
      : textForNotification;
