...
const ATTACHMENT_NOTE_TEXT = "Публикацията съдържа само прикачени файлове без текст в страницата.";
...
export async function fetchPostLinksFromFeed(
  ...
  while ((m = entryRe.exec(xml)) !== null) {
    ...
    const linkHref = decodeHtmlEntities(
      entryXml.match(/<atom:link[^>]*href="([^"]+)"[^>]*type="text\/html"[^>]*\/?/i)?.[1]?.trim() ?? "",
    );
    const portalContextPath = decodeHtmlEntities(
      entryXml.match(/<wplc:field id="portalcontextpath">([\s\S]*?)<\/wplc:field>/)?.[1]?.trim()
        ?? DEFAULT_PORTAL_CONTEXT_PATH,
    );
    ...
    const url = buildCanonicalDetailUrl(contentPath, linkHref, portalContextPath);
...
async function fetchFeedXmlWithRetry(
  ...
  for (let attempt = 1; attempt <= FEED_FETCH_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (!response.ok) {
        if (shouldRetryStatus(response.status) && attempt < FEED_FETCH_MAX_ATTEMPTS) {
          const waitMs = calculateRetryDelayMs(attempt);
          await response.body?.cancel?.().catch(() => undefined);
          logger.debug("Retrying Serdika feed fetch after retryable HTTP status", {
      ...
      const waitMs = calculateRetryDelayMs(attempt);
      logger.debug("Retrying Serdika feed fetch after transient network error", {
        sourceType: SOURCE_TYPE,
        sectionKeyword,
        attempt,
        maxAttempts: FEED_FETCH_MAX_ATTEMPTS,
...
function buildCanonicalDetailUrl(
  ...
  const normalizedPortalContextPath = portalContextPath || DEFAULT_PORTAL_CONTEXT_PATH;
...
async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
    try {
      ...
      if (!shouldRetryStatus(response.status) || attempt === options.maxAttempts) {
        return response;
      }
      ...
      logger.debug("Retrying Serdika detail fetch after retryable HTTP status", {
        sourceType: SOURCE_TYPE,
        url,
        attempt,
      ...
      const waitMs = calculateRetryDelayMs(attempt);
      logger.debug("Retrying Serdika detail fetch after transient network error", {
        sourceType: SOURCE_TYPE,
        url,
        attempt,
        maxAttempts: options.maxAttempts,
...
function shouldRetryError(error: unknown): boolean {
  ...
  return /ECONNRESET|ETIMEDOUT|timed out|fetch failed/i.test(getErrorMessage(error));
}
...
function extractPostDetailsFromHtml(
  ...
  const titleRegion = extractElementOuterHtmlByClass(html, "div", "content-wrapper") ?? html;
  const title = cleanText(
    extractFirstTagText(titleRegion, "h2") ?? extractFirstTagText(html, "h2") ?? "",
  );
