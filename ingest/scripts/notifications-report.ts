...
function extractHeatmapPoints(
  ...
  for (const feature of features) {
    ...
    const { type, coordinates } = geom as {
      type?: unknown;
      coordinates?: unknown;
    };
...
`,
  ...
  .action(async (opts: { dryRun?: boolean }) => {
    try {
      ...
      const { saveNotificationsReportSnapshot } =
        await import("@/lib/notifications/report-store");
      ...
      try {
        ...
        for (const match of matches) {
          ...
          const msgId =
            typeof match.messageId === "string" ? match.messageId : "";
          const source =
            ...
            typeof (match.messageSnapshot as Record<string, unknown>).source ===
              "string"
              ? ((match.messageSnapshot as Record<string, unknown>)
                  .source as string)
              : "(unknown)");
        ...
        const snapshot: NotificationsReportSnapshot = {
          ...
          heatmap: {
            ...
            clicked: buildHeatmapMode(matches, messageMap, (m) =>
              Boolean(m.clickedAt),
            ),
            opened: buildHeatmapMode(matches, messageMap, (m) =>
              Boolean(m.openedAt),
            ),
          },
        };
        ...
        console.log(
          `  Heatmap all: ${snapshot.heatmap.all.points.length} points (hidden: ${snapshot.heatmap.all.hiddenForPrivacy})`,
        );
        console.log(
          `  Heatmap clicked: ${snapshot.heatmap.clicked.points.length} points (hidden: ${snapshot.heatmap.clicked.hiddenForPrivacy})`,
        );
        console.log(
          `  Heatmap opened: ${snapshot.heatmap.opened.points.length} points (hidden: ${snapshot.heatmap.opened.hiddenForPrivacy})`,
        );
